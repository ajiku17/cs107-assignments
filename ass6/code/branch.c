            ...
            AccountAmount initialAmount)
{
  ...
  for (int i = 0; i < numBranches; i++) {
    ...
    branch->branchDataIsBeingProcessed = malloc(sizeof(sem_t));
    sem_init(branch->branchDataIsBeingProcessed, 0, 1);
...
Branch_Balance(Bank *bank, BranchID branchID, AccountAmount *balance)
{
  ...
  sem_wait(bank->branches[branchID].branchDataIsBeingProcessed);
  ...
  *balance = bank->branches[branchID].balance;  Y;
  ...
  sem_post(bank->branches[branchID].branchDataIsBeingProcessed);
