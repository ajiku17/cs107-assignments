...
;;My compare function which compares rated points based on their rating and is passed to the partition function
;;-----------------------
(define (compare_points p1 p2)
        (if (< (car p1) (car p2)) #t #f))
;;-----------------------
...
(define (partition pivot num-list)
  (if (null? num-list) '(() ())
	...
	(if (compare_points (car num-list) pivot)
...
;--1--
(define (intersection-points circle_list)
  (if (null? circle_list) '() 
    (append (apply append (map (lambda (second_circle) (intersect (car circle_list) second_circle))  (cdr circle_list))) 
        (intersection-points (cdr circle_list)) ) ))


;--2--
(define (distance-product circle circle_list)
        (if (null? circle_list) 0
          (apply * (map (lambda (second_circle) 
          							(if (equal? circle second_circle) 1
          										(dist circle second_circle))) circle_list) )))

;--3--
(define (rate-points point_list)
        (map (lambda (point) (list (distance-product point point_list) point)) point_list))
...
;--5--
(define (clumped-points point_list)
		(prefix-of-list (map cadr (sort-points (rate-points point_list))) (quotient (length point_list) 2)))

;--6--
(define (average ls)
		(/ (apply + ls) (length ls)))

(define (average-point point_list)
		(let ((avg_point (list (average (map car point_list)) (average (map cadr point_list)))))
		(list (distance-product avg_point point_list) avg_point) ))
