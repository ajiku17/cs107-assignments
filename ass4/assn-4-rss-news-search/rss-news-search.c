...
static void PullAllNewsItems(urlconnection *urlconn, data_t* data);
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, data_t* data);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, data_t* data);
static void QueryIndices(data_t* data);
static void ProcessResponse(const char *word, data_t* data);
static bool WordIsWellFormed(const char *word);
static void initStopWords(hashset* stopWords);
...
static const char* const stopWordsTextFile = "data/stop-words.txt";

static const char *const kNewLineDelimiters = "\r\n";

static const int kApproximateWordCount = 10007;
static const int kApproximateArticleCount = 1009;
...
int stringCompare(const void* vp1, const void* vp2){
  char* s1 = *(char**)vp1;
  char* s2 = *(char**)vp2;
  return strcasecmp(s1, s2);
}

static int pairHash(const void* vp, int numBuckets){
  pair* p = (pair*)vp;
  
  int i;
  unsigned long hashcode = 0;
  
  for (i = 0; i < strlen(p->keyword); i++)  
    hashcode = hashcode * kHashMultiplier + tolower((p->keyword)[i]);  
  
  return hashcode % numBuckets;
}

int pairCompare(const void* vp1, const void* vp2){
  pair* p1 = (pair*)vp1;
  char* s2 = *(char**)vp2;
  return strcasecmp(p1->keyword, s2);
}

void pairFree(void* vp){
  pair* p = (pair*)vp;
  free(p->keyword);
  VectorDispose(p->articleInfos);
  free(p->articleInfos);
}

static int articleHash(const void* vp, int numBuckets){
  article* a = (article*)vp;
  int i;
  unsigned long hashcode = 0;
  for (i = 0; i < strlen(a->URL); i++)  
    hashcode = hashcode * kHashMultiplier + tolower((a->URL)[i]);  
  
  return hashcode % numBuckets;
}

int articleCompare(const void* vp1, const void* vp2){
  article* a1 = (article*)vp1;
  article* a2 = (article*)vp2;
  int URLres = strcmp(a1->URL, a2->URL);
  if(URLres != 0){
    if(strcasecmp(a1->articleTitle, a2->articleTitle) == 0){
      int i = 0;
      while((a1->URL)[i] && (a2->URL)[i]){
        if((a1->URL)[i] != (a2->URL)[i])break;
        if((a1->URL)[i] == '/' && (a2->URL)[i] == '/')return 0;
        i++;
      }
    }
  }
...
int articleInfoCompare(const void* vp1, const void* vp2){
  char* s1 = *(char**)vp1;
  articleInfo* a1 = (articleInfo*)vp2;
  article* A1 = a1->artPointer;
  char* s2 = A1->URL;
  return strcmp(s1, s2);
}


static int occuranceCompare(const void* vp1, const void* vp2){
  articleInfo* a1 = (articleInfo*)vp1;
  articleInfo* a2 = (articleInfo*)vp2;
  if(a1->numOccurances > a2->numOccurances){
    return -1;
  }else if(a1->numOccurances == a2->numOccurances){
    return 0;
  }else{
    return 1;
  }
...
int main(int argc, char **argv)
{
  ...
  data->mapOfWords = malloc(sizeof(hashset));
  data->setOfArticles = malloc(sizeof(hashset));
  HashSetNew(data->stopWords, sizeof(char*), kApproximateWordCount, StringHash, stringCompare, stringFree);
  initStopWords(data->stopWords);

  HashSetNew(data->mapOfWords, sizeof(pair), kApproximateWordCount, pairHash, pairCompare, pairFree);
  HashSetNew(data->setOfArticles, sizeof(article), kApproximateArticleCount, articleHash, articleCompare, articleFree);
  ...
  HashSetDispose(data->mapOfWords);
  HashSetDispose(data->setOfArticles);

  free(data->stopWords);
  free(data->mapOfWords);
  free(data->setOfArticles);
...
/*Function: initStopWords
  -----------------------
  Reads words from a text file specified by stopWordsTextFile and
  populates a given hashset with words that way too common to be
  taken seriously for a query
*/
static void initStopWords(hashset* stopWords){
  streamtokenizer st;

  FILE * infile;
  infile = fopen(stopWordsTextFile, "r");
...
static void ProcessFeedFromFile(char *fileName, data_t* data)
{
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription, (const char *)fileName, data);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew doesn't open one..
  fclose(infile);
...
static void ProcessFeed(const char *remoteDocumentName, data_t* data)
{
  ...
  if(!strncmp(filePrefix, remoteDocumentName, strlen(filePrefix))){
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(filePrefix), data);
    return;
  }
  ...
  switch (urlconn.responseCode) {
      ...
      case 301: 
      case 302: 
                ProcessFeed(urlconn.newUrl, data);
                break;
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, data_t* data)
{
  ...
  switch (urlconn.responseCode) {
      ...
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
        ParseArticle(articleTitle, articleDescription, urlconn.newUrl, data);
        break;
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, data_t* data)
{
  ...
  newArticle.description = strdup(unused);

  HashSetEnter(data->setOfArticles, &newArticle);

  article* currentArticle = HashSetLookup(data->setOfArticles, &newArticle);
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        if(HashSetLookup(data->stopWords, &copy) == NULL){
          pair* foundWord = HashSetLookup(data->mapOfWords, &copy);
          if(foundWord != NULL){
            int foundArticle = VectorSearch(foundWord->articleInfos, &articleURL, articleInfoCompare, 0, false);
            if(foundArticle != -1){
              articleInfo* currArticleInfo = VectorNth(foundWord->articleInfos, foundArticle);
              (currArticleInfo->numOccurances)++;
            }else{
              articleInfo newArticleInfo;
              newArticleInfo.numOccurances = 1;
              newArticleInfo.artPointer = currentArticle;
              VectorAppend(foundWord->articleInfos, &newArticleInfo);
            }
          }else{
            pair newPair;
            newPair.keyword = strdup(word);
            newPair.articleInfos = malloc(sizeof(vector));
            VectorNew(newPair.articleInfos, sizeof(articleInfo), NULL , 1);
            articleInfo newArticleInfo;
            newArticleInfo.artPointer = currentArticle;
            newArticleInfo.numOccurances = 1;
            VectorAppend(newPair.articleInfos, &newArticleInfo);
            HashSetEnter(data->mapOfWords, &newPair);
          }
...
static void ProcessResponse(const char *word, data_t* data)
{
  if (WordIsWellFormed(word)) {
    ...
    pair* found = HashSetLookup(data->mapOfWords, &word);
    if(found != NULL){
      vector* connections = found->articleInfos;
      VectorSort(connections, occuranceCompare);
      for(int i = 0; i < min(VectorLength(connections), 10); i++){
        char* s = "";
        if(((articleInfo*)(VectorNth(connections, i)))->numOccurances > 1) s = "s";
        article* currArticle = ((articleInfo*)(VectorNth(connections, i)))->artPointer;
        printf("%d.) \"%s\" [search term occurs %d time%s]\n", i + 1, currArticle->articleTitle, 
                              ((articleInfo*)(VectorNth(connections, i)))->numOccurances, s);        
        printf("\"%s\"\n", currArticle->URL);
      }
    }else{
