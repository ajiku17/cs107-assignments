...
# everything is almost the same as the starter code. instead of returing just the optimal score, i return
# alignments too in form of a 3 argument list
def optimal(strand1, strand2, saved):
	toSearch = strand1 + "|" + strand2
	if toSearch in saved:
		return saved[toSearch]
	...
	if len(strand1) == 0:
		score = len(strand2)* -2
		for i in xrange(0, len(strand2)):
			alignment1.append(' ')
			alignment2.append(strand2[i])

		tri = [score, alignment1, alignment2]
		return tri

	if len(strand2) == 0:
		score = len(strand1)* -2
		for i in xrange(0, len(strand1)):
			alignment1.append(strand1[i])
			alignment2.append(' ')

		tri = [score, alignment1, alignment2]
		return tri

	bestWith = optimal(strand1[1:], strand2[1:], saved)
	
	alignment1 = [strand1[0]] + bestWith[1]
	alignment2 = [strand2[0]] + bestWith[2]
	...
	best = [score, alignment1, alignment2]

	bestWithout = optimal(strand1, strand2[1:], saved)
	if bestWithout[0] - 2 > score:
		newScore = bestWithout[0] - 2
		newAlignment1 = [' '] + bestWithout[1]
		newAlignment2 = [strand2[0]] + bestWithout[2]
		best = [newScore, newAlignment1, newAlignment2]

	bestWithout = optimal(strand1[1:], strand2, saved)
	if bestWithout[0] - 2 > score:
		newScore = bestWithout[0] - 2
		newAlignment1 = [strand1[0]] + bestWithout[1]
		newAlignment2 = [' '] + bestWithout[2]
		best = [newScore, newAlignment1, newAlignment2]
...
def main():
	...
	while (True):
		...
		if answer == "no\n": break
		strand1 = generateRandomDNAStrand(198, 200)
		strand2 = generateRandomDNAStrand(198, 200)
		# strand1 = 'TGTAAGAGAT'
		# strand2 = 'CTAATCGA'
		...
		best = optimal(strand1, strand2, memo)
		plus = minus = al1 = al2 = ''
		printAlignment(best[0])
		for i in xrange(0, len(best[1])):
			if best[1][i] == ' ' or best[2][i] == " ":
				plus += ' '
				minus += '2'
			elif best[1][i] == best[2][i]:
				plus += '1'
				minus += ' '
			else:
				plus += ' '
		...
		test(best[0], plus, minus, al1, al2)
